...
class SummarizeJob < ApplicationJob
  ...
  def perform(chat_result_id)
    ...
    if result
      ...
      # 後続の非同期処理をenqueue
      TagProfilingJob.perform_later(character.id, result[:file_path])
      if chat_result.usage&.dig("full_log_path")
        SleepPhaseJob.perform_later(character.id, chat_result.usage["full_log_path"])
      end
    else
      chat_result.fail!("Reflection generation returned nil")
    end
  rescue => e
    Rails.logger.error("[SummarizeJob] Error: #{e.message}\n#{e.backtrace&.first(5)&.join("\n")}")
  ...
  def build_usage_tracker(user, character)
    lambda { |model, usage|
      begin
        ...
        Rails.logger.warn("[SummarizeJob] Usage tracking failed: #{e.message}")
      end
    }
  end
end
