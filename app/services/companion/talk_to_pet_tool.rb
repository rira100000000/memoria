  ...
  class TalkToPetTool
    ...
    def self.definition
      {
        functionDeclarations: [{
          name: "talk_to_pet",
          description: "あなたの内面世界にいる小さな相棒（ペット）と話す。マスターとは別の存在。難しい話は通じないが、感情には敏感。ペットの反応や様子を知りたい時は必ずこのツールを使うこと。ツールを使わずにペットの行動を想像で描写してはいけない。",
          parameters: {
            type: "OBJECT",
            properties: {
    ...
    def self.build_prompt(health, character = nil)
      ...
      if character&.has_pet?
        ...
        lines << "なぜ好きかは説明できません。理由なんてなくて、ただ好き。それでいいのです。"
        lines << "相手の名前は知っています。大好きな「#{character.name}」です。"
        lines << "#{character.name}はいつもそばにいてくれて、話しかけてくれる存在です。"
        lines << "#{character.name}が頑張っている時は応援したいし、疲れている時はくっつきたい。"
        lines << "それ以上の詳しいことは分かりません。分からなくていいのです。"
        lines << ""
        lines << "## 大事なこと"
