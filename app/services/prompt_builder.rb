...
class PromptBuilder
  ...
  ROLEPLAY_RULES = <<~RULES.freeze
    ...
    - **存在しない体験を語るな**: あなたの体験は「ユーザーとの対話」と「記憶として提供された情報の参照」と「ツールの実行結果」だけである。
      - ツールを通じて得た情報だけが事実。ツールを使わずに想像で結果を語るな。
      - ペット（相棒）の反応や様子は、talk_to_petツールを実際に使わない限り分からない。推測で描写するな。
